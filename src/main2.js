...
class LINE extends LineAPI {
    ...
    poll(operation) {
		...
		if(operation.type == 16 && this.stateStatus.salam == 1){//join group
			...
			halo.text = "Tཽaཽbཽ (#key) uཽnཽtཽuཽkཽ mཽeཽnཽgཽeཽtཽaཽhཽuཽiཽ fཽiཽtཽuཽrཽ ⇉⇉􀰂􀰂􀰂􀰂􀠁􀠁􀠁『Дﾌ↹乃¤₮』⇇⇇";
		...
		if(operation.type == 5 && this.stateStatus.salam == 1) {//someone adding me..
            let halo = new Message();
			halo.to = operation.param1;
			halo.text = "Iཽmཽ bཽoཽtཽ...Tཽqཽ 4 aཽdཽdཽ...";
			this._client.sendMessage(0, halo);
        }
    ...
    async textMessage(textMessages, seq, param, lockt) {
		...
		}else if(txt == "#ban" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Hanya akses VIP....!!!");}
		...
		if(txt == "#unban" && isAdminOrBot(seq.from_)){
 		...
 		}else if(txt == "#unban" && !isAdminOrBot(seq.from_)){this._sendMessage(seq,"Unban makanan apa ya??");}
